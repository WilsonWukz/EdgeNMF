...
from nmf import Hypersurface_nmf
    ...
    for delta in deltas:
        for p, r in pr_lst:
            ...
            (W, H, WH, step), timeusing, peak_mem = measure_performance(
                Hypersurface_nmf, X_train, rank, steps, tol, delta, eps, verbose
            )
            ...
            record_HC_Yale.append(((p,r), delta, acc, nmi, rre, tol, step, seed, timeusing, peak_mem, Algo, Data))

df_HC_Yale = pd.DataFrame(record_HC_Yale, columns=['(p,r)','delta','acc','nmi','rre','tol','step','seed', 'time', 'memory', 'Algo', 'Data'])
print(df_HC_Yale)
...
print(summary)
df_HC_Yale.to_excel(f'{Algo}_{Data}_Raw.xlsx', index=True)
