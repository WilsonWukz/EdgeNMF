    ...
    for lr in lrs:
        for p, r in pr_lst:
            ...
            (W, H, WH, step), timeusing, peak_mem = measure_performance(
                L2_nmf, X_train, K, lr, steps, tol, verbose=False
            )
            ...
            record_L2_Yale.append(((p,r), acc, nmi, rre, lr, tol, step, seed, timeusing, peak_mem, Algo, Data))

df_L2_Yale = pd.DataFrame(record_L2_Yale, columns=['(p,r)','acc','nmi','rre','lr','tol','step','seed', 'time', 'memory', 'Algo', 'Data'])
print(df_L2_Yale)
...
print(summary)
df_L2_Yale.to_excel(f'{Algo}_{Data}_Raw.xlsx', index=True)
