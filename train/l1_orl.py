...
for seed in seeds:
    ...
    for lambda_h, lambda_w in lambdas:
        for p, r in pr_lst:
            ...
            record_L1_ORL.append(((p,r), (lambda_h,lambda_w), acc, nmi, rre, step, seed, timeusing, peak_mem, Algo, Data))

cols = ['(p,r)', 'lambdas', 'acc', 'nmi', 'rre', 'step', 'seed', 'time', 'memory', 'Algo', 'Data']
df_L1_ORL = pd.DataFrame(record_L1_ORL, columns=cols)

summary_lambdas = df_L1_ORL.groupby(['(p,r)', 'lambdas']).agg(
    ACC_mean=('acc', 'mean'),
    Time_mean=('time', 'mean'),
...
print(summary)
df_L1_ORL.to_excel(f'{Algo}_{Data}_Raw.xlsx', index=True)
